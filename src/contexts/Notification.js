...
const Notification = ({ message, show, duration = 5000 }) => {
    ...
    const notificationStyle = message === 'Already in the list' ? 'bg-red-500'  : 'bg-green-500';  

    return (
        <div className={`fixed bottom-4 right-4 ${notificationStyle} text-white px-4 py-2 rounded shadow-lg`}>
            {message}
        </div>
    );
};
